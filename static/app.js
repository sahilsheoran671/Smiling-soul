...
const usernameInput = "ok";
//const appointmentid=document.getElementById('appointmentxyz');
const appointmentid="ok";
...
function connectButtonHandler(event) {
    ...
    if (!connected) {
        ...
        let id=appointmentid.value
        /*// if (!username) {
        //     alert('Enter your name before connecting');
        //     return;
        // }
        // if (!id) {
        //     alert('Enter your id before connecting');
        //     return;
        // } */
        button.disabled = true;
        button.innerHTML = 'Connecting...';
        connect(username,id).then(() => {
...
function connect(username,appointmentxyz) {
    let promise = new Promise((resolve, reject) => {
        // get a token from the back end
        let data;
        fetch('/video', {
            method: 'POST',
            body: JSON.stringify({'username': username,'appointmentxyz':appointmentxyz})
